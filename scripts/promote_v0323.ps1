...
$PackagedSelfTest = Join-Path $CandidateRoot 'artifacts\packaged-self-test-v0323.json'
$PackagedCompactResource = Join-Path $CandidateRoot 'artifacts\packaged-compact-resource-v0323.json'
$PackagedWindowsStartup = Join-Path $CandidateRoot 'artifacts\packaged-windows-startup-v0323.json'
...
if ([int]$compactResource.schemaVersion -ne 1 -or
    ...
    throw 'Packaged compact resource report did not satisfy the v0.3.23 release gate.'
}
...
if ([int]$windowsStartup.schemaVersion -ne 1 -or
    ...
    throw 'Packaged Windows startup report did not satisfy the v0.3.23 release gate.'
}
...
$AdditionalFiles = @(
    ...
    'artifacts\packaged-compact-resource-v0323.json',
    'artifacts\packaged-windows-startup-v0323.json'
)

& $Engine `
    -FormalRoot $FormalRoot `
    -ReleaseVersion $ReleaseVersion `
    -PromotionScript 'scripts\promote_v0323.ps1' `
    -PackagedReport 'artifacts\packaged-self-test-v0323.json' `
