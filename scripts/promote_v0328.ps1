...
$ReleaseVersion = '0.3.28'
$FileVersion = '0.3.28.0'
$PackagedSelfTest = Join-Path $CandidateRoot 'artifacts\packaged-self-test-v0328.json'
$PackagedCompactResource = Join-Path $CandidateRoot 'artifacts\packaged-compact-resource-v0328.json'
$PackagedWindowsStartup = Join-Path $CandidateRoot 'artifacts\packaged-windows-startup-v0328.json'
...
if ([int]$compactResource.schemaVersion -ne 1 -or
    ...
    throw 'Packaged compact resource report did not satisfy the v0.3.28 release gate.'
}
...
if ([int]$windowsStartup.schemaVersion -ne 1 -or
    ...
    throw 'Packaged Windows startup report did not satisfy the v0.3.28 release gate.'
}
...
$AdditionalFiles = @(
    ...
    'artifacts\packaged-compact-resource-v0328.json',
    'artifacts\packaged-windows-startup-v0328.json'
)

& $Engine `
    -FormalRoot $FormalRoot `
    -ReleaseVersion $ReleaseVersion `
    -PromotionScript 'scripts\promote_v0328.ps1' `
    -PackagedReport 'artifacts\packaged-self-test-v0328.json' `
    -AdditionalFiles $AdditionalFiles

# The transaction engine verifies its complete staged tree.  Keep an explicit
# v0.3.28 checksum fence for the newly changed production, policy and verifier
# files so a successful promotion can never leave the formal source tree on a
# mixed revision.
