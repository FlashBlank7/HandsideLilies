...
$PackagedSelfTest = Join-Path $CandidateRoot 'artifacts\packaged-self-test-v0332.json'
$PackagedCompactResource = Join-Path $CandidateRoot 'artifacts\packaged-compact-resource-v0332.json'
$PackagedWindowsStartup = Join-Path $CandidateRoot 'artifacts\packaged-windows-startup-v0332.json'
...
if ([int]$compactResource.schemaVersion -ne 1 -or
    ...
    throw 'Packaged compact resource report did not satisfy the v0.3.32 release gate.'
}
...
if ([int]$windowsStartup.schemaVersion -ne 1 -or
    ...
    throw 'Packaged Windows startup report did not satisfy the v0.3.32 release gate.'
}
...
& $Engine `
    ...
    -PromotionScript 'scripts\promote_v0332.ps1' `
    -PackagedReport 'artifacts\packaged-self-test-v0332.json' `
    -AdditionalFiles $AdditionalFiles

# Keep an explicit v0.3.32 checksum fence for every source, gate and artwork
