...
$FileVersion = '0.3.33.0'
$PackagedSelfTest = Join-Path $CandidateRoot 'artifacts\packaged-self-test-v0333.json'
$PackagedCompactResource = Join-Path $CandidateRoot 'artifacts\packaged-compact-resource-v0333.json'
$PackagedWindowsStartup = Join-Path $CandidateRoot 'artifacts\packaged-windows-startup-v0333.json'
$CandidateDist = Join-Path $CandidateRoot 'dist\LiliesInTheBox'
$CandidateExe = Join-Path $CandidateDist 'LiliesInTheBox.exe'
$HistoricalV0332 = Join-Path $CandidateRoot 'scripts\promote_v0332.ps1'
$HistoricalV0332Sha256 = '8077086c0b8c450633a44b73808593a1b0d6d3ef5425cdd7decad3d7226d1a2f'

if (-not (Test-Path -LiteralPath $HistoricalV0332 -PathType Leaf)) {
    throw "Historical v0.3.32 promotion wrapper is missing: $HistoricalV0332"
}
$historicalV0332Actual = (
    Get-FileHash -LiteralPath $HistoricalV0332 -Algorithm SHA256
).Hash.ToLowerInvariant()
if ($historicalV0332Actual -cne $HistoricalV0332Sha256) {
    throw "Historical v0.3.32 promotion wrapper changed: $historicalV0332Actual"
}
...
if ([int]$compactResource.schemaVersion -ne 1 -or
    ...
    throw 'Packaged compact resource report did not satisfy the v0.3.33 release gate.'
}
...
if ([int]$windowsStartup.schemaVersion -ne 1 -or
    ...
    throw 'Packaged Windows startup report did not satisfy the v0.3.33 release gate.'
}
...
$AdditionalFiles = @(
    ...
    'artifacts\pose-click-mask-v0333.json',
    ...
    'artifacts\packaged-compact-resource-v0333.json',
    'artifacts\packaged-windows-startup-v0333.json'
)

& $Engine `
    -FormalRoot $FormalRoot `
    -ReleaseVersion $ReleaseVersion `
    -PromotionScript 'scripts\promote_v0333.ps1' `
    -PackagedReport 'artifacts\packaged-self-test-v0333.json' `
    -AdditionalFiles $AdditionalFiles

# Keep an explicit v0.3.33 checksum fence for every source, gate and artwork
