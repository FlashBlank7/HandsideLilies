...
$ReleaseVersion = '0.3.37'
$FileVersion = '0.3.37.0'
$PackagedSelfTest = Join-Path $CandidateRoot 'artifacts\packaged-self-test-v0337.json'
$PackagedCompactResource = Join-Path $CandidateRoot 'artifacts\packaged-compact-resource-v0337.json'
$PackagedWindowsStartup = Join-Path $CandidateRoot 'artifacts\packaged-windows-startup-v0337.json'
$PoseClickMaskReport = Join-Path $CandidateRoot 'artifacts\pose-click-mask-v0337.json'
...
$HistoricalV0336 = Join-Path $CandidateRoot 'scripts\promote_v0336.ps1'
$HistoricalV0336Sha256 = '433f8b8eee017759c5904ae074063f1965ffefcb4c0a9d60c02996901df3b8d9'

if (-not (Test-Path -LiteralPath $HistoricalV0336 -PathType Leaf)) {
    throw "Historical v0.3.36 promotion wrapper is missing: $HistoricalV0336"
}
$historicalV0336Actual = (
    Get-FileHash -LiteralPath $HistoricalV0336 -Algorithm SHA256
).Hash.ToLowerInvariant()
if ($historicalV0336Actual -cne $HistoricalV0336Sha256) {
    throw "Historical v0.3.36 promotion wrapper changed: $historicalV0336Actual"
}
...
function Assert-PoseClickMaskReport([object]$Report) {
...
Assert-ReleaseArtifactFreshness `
    ...
    @(
        $CandidateExe,
        (Join-Path $CandidateRoot 'qml\V03PetBody.qml'),
        (Join-Path $CandidateRoot 'qml\V03PetPoseResolver.qml'),
        (Join-Path $CandidateRoot 'scripts\verify_pose_click_masks.py'),
        (Join-Path $CandidateRoot 'themes\first-encounter\theme.json'),
        (Join-Path $CandidateRoot 'themes\first-encounter\assets\lilith-pose-perch-prone-v1.png'),
        (Join-Path $CandidateRoot 'themes\first-encounter\assets\lilith-pose-title-sit-v1.png'),
        (Join-Path $CandidateRoot 'themes\first-encounter\assets\lilith-pose-edge-peek-v1.png'),
        (Join-Path $CandidateRoot 'themes\first-encounter\assets\lilith-pose-listening-live-v1.png'),
        (Join-Path $CandidateRoot 'themes\first-encounter\assets\lilith-pose-focus-kneel-v1.png')
    ) `
    'poseClickMask'
...
if ([int]$compactResource.schemaVersion -ne 1 -or
    ...
    throw 'Packaged compact resource report did not satisfy the v0.3.37 release gate.'
}
...
if ([int]$windowsStartup.schemaVersion -ne 1 -or
    ...
    throw 'Packaged Windows startup report did not satisfy the v0.3.37 release gate.'
}
...
& $Engine `
    ...
    -PromotionScript 'scripts\promote_v0337.ps1' `
    -PackagedReport 'artifacts\packaged-self-test-v0337.json' `
    -AdditionalFiles $AdditionalFiles

# Keep an explicit v0.3.37 checksum fence for every source, gate and artwork
