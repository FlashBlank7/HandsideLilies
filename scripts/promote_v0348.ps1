...
$ReleaseVersion = '0.3.48'
$FileVersion = '0.3.48.0'
$PackagedSelfTest = Join-Path $CandidateRoot 'artifacts\packaged-self-test-v0348.json'
$PackagedCompactResource = Join-Path $CandidateRoot 'artifacts\packaged-compact-resource-v0348.json'
$PackagedWindowsStartup = Join-Path $CandidateRoot 'artifacts\packaged-windows-startup-v0348.json'
$PoseClickMaskReport = Join-Path $CandidateRoot 'artifacts\pose-click-mask-v0348.json'
...
if ([int]$compactResource.schemaVersion -ne 1 -or
    ...
    throw 'Packaged compact resource report did not satisfy the v0.3.48 release gate.'
}
...
if ([int]$windowsStartup.schemaVersion -ne 1 -or
    ...
    throw 'Packaged Windows startup report did not satisfy the v0.3.48 release gate.'
}
...
& $Engine `
    ...
    -PromotionScript 'scripts\promote_v0348.ps1' `
    -PackagedReport 'artifacts\packaged-self-test-v0348.json' `
    -AdditionalFiles $AdditionalFiles

# Keep an explicit v0.3.48 checksum fence for every source and release gate
# changed in this release, including the layered Win32 drag proxy, local
# gesture batching and generation-guarded preview lifecycle.
