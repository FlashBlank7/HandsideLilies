...
$ReleaseVersion = '0.3.49'
$FileVersion = '0.3.49.0'
$PackagedSelfTest = Join-Path $CandidateRoot 'artifacts\packaged-self-test-v0349.json'
$PackagedCompactResource = Join-Path $CandidateRoot 'artifacts\packaged-compact-resource-v0349.json'
$PackagedWindowsStartup = Join-Path $CandidateRoot 'artifacts\packaged-windows-startup-v0349.json'
$PoseClickMaskReport = Join-Path $CandidateRoot 'artifacts\pose-click-mask-v0349.json'
...
if ([int]$compactResource.schemaVersion -ne 1 -or
    ...
    throw 'Packaged compact resource report did not satisfy the v0.3.49 release gate.'
}
...
if ([int]$windowsStartup.schemaVersion -ne 1 -or
    ...
    throw 'Packaged Windows startup report did not satisfy the v0.3.49 release gate.'
}
...
& $Engine `
    ...
    -PromotionScript 'scripts\promote_v0349.ps1' `
    -PackagedReport 'artifacts\packaged-self-test-v0349.json' `
    -AdditionalFiles $AdditionalFiles

# Keep an explicit v0.3.49 checksum fence for every source and release gate
# changed in this release, including high-DPI proxy capture, gesture-fenced
# snapshots, hot-path cleanup, reflow coalescing and the one-frame preview bridge.
