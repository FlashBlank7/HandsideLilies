...
def test_v0348_active_version_points_are_aligned() -> None:
    markers = {
        "pyproject.toml": 'version = "0.3.48"',
        "uv.lock": 'version = "0.3.48"',
        "src/lilies_in_the_box.egg-info/PKG-INFO": "Version: 0.3.48",
        "src/lilies/__init__.py": '__version__ = "0.3.48"',
        "src/lilies/app.py": 'app.setApplicationVersion("0.3.48")',
        "src/lilies/core/codex_subscription.py": 'CLIENT_VERSION = "0.3.48"',
        "src/lilies/connectors/slack_websocket.py": (
            '_USER_AGENT = "lilies-in-the-box/0.3.48"'
        ),
        "scripts/install_windows.ps1": "version = '0.3.48'",
    }
    ...
    for marker in ("(0, 3, 48, 0)", '"0.3.48.0"', '"0.3.48"'):
        assert marker in version_info


def test_v0348_report_build_install_and_verify_pointers_are_aligned() -> None:
    pointers = {
        "scripts/build_windows.ps1": "packaged-self-test-v0348.json",
        "scripts/verify_codex_subscription_smoke.py": ".codex-subscription-smoke-v0348",
        "scripts/verify_packaged_windows_startup.ps1": "packaged-windows-startup-v0348.json",
        "scripts/verify_packaged_compact_resources.ps1": "packaged-compact-resource-v0348.json",
        "scripts/verify_pose_click_masks.py": "pose-click-mask-v0348.json",
        "tests/test_pose_click_mask_qml_v0329.py": "pose-click-mask-v0348.json",
        "tests/test_windows_startup_probe_contract.py": "packaged-windows-startup-v0348.json",
    }
    ...
    for report in (
        "packaged-self-test-v0348.json",
        "packaged-compact-resource-v0348.json",
        "packaged-windows-startup-v0348.json",
        "pose-click-mask-v0348.json",
    ):
        assert report in build
    assert "Built Lilies v0.3.48" in build

    compact = _read("scripts/verify_packaged_compact_resources.ps1")
    assert "$ExpectedApplicationVersion = '0.3.48'" in compact
    assert "packaged-self-test-v0348.json" in compact

    installer = _read("scripts/install_windows.ps1")
    assert "$ExpectedFileVersion = '0.3.48.0'" in installer
    assert "$ExpectedProductVersion = '0.3.48'" in installer


def test_v0348_release_evidence_is_bound_to_the_packaged_candidate() -> None:
    ...
    wrapper = _read("scripts/promote_v0348.ps1")
...
def test_v0348_wrapper_fences_all_immutable_historical_wrappers() -> None:
    ...
    assert "$ReleaseVersion = '0.3.48'" in wrapper
    assert "$FileVersion = '0.3.48.0'" in wrapper
    assert "packaged-self-test-v0348.json" in wrapper
    assert "packaged-compact-resource-v0348.json" in wrapper
    assert "packaged-windows-startup-v0348.json" in wrapper
    assert "pose-click-mask-v0348.json" in wrapper
    assert r"-PromotionScript 'scripts\promote_v0348.ps1'" in wrapper
    assert r"-PackagedReport 'artifacts\packaged-self-test-v0348.json'" in wrapper


def test_v0348_additional_files_and_checksum_fence_cover_release_delta() -> None:
    additional, fence = _release_file_sets(_read("scripts/promote_v0348.ps1"))
    required = {
        ".gitignore",
        "LiliesInTheBox.spec",
        "README.md",
...
def test_v0348_readme_is_current_and_documents_release_gates() -> None:
    readme = _read("README.md")
    assert readme.index("### v0.3.48") < readme.index("### v0.3.47")
    current = readme[readme.index("### v0.3.48") : readme.index("### v0.3.47")]
    for marker in (
        "UpdateLayeredWindow",
        "FrameAnimation",
        "resize/accessory/action",
        "schema 3",
        "QML",
        "Win32",
        "QTimer",
        "generation",
        "v0.3.48",
        "promote_v0348.ps1 -ValidateOnly",
    ):
        assert marker in current
    assert "promote_v0348.ps1 -ValidateOnly" in readme
